...
namespace = 'default'
in_cluster = False # False because Airflow runs in Docker, not in the K8s cluster
...
volume = k8s.V1Volume(
        name='test',
        host_path=k8s.V1HostPathVolumeSource(path='/workspaces/af-demo/tmp')
        )
with dag:
    ...
    run_data_ingest = KubernetesPodOperator(
        namespace=namespace,
        cluster_context='kind-k8s-playground',
